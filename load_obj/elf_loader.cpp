...
int lookup_string_section(u8 *section_addr, u32 section_num)
{
  ...
  for (int i=0 ; i < section_num ; ++i)
  {
    if (shdr->sh_type == 3) // string section
    {
      ...
      u8 *ptr = hello_addr+shdr->sh_offset;
      int ptr_index=0;
      int total_len=0;
...
{
  printf("%s elf_filename\n", fn);
}

const char *elf_class_str(int elf_c)
{
  const char *str[] = {"elf32", "elf64"};
...
void print_elf_hdr(Elf32Ehdr *hdr)
{
  printf("hdr->e_type: %d\n", hdr->e_type);
  printf("class: %s\n", elf_class_str(hdr->e_ident[4]));
  printf("hdr->e_phnum: %d\n", hdr->e_phnum);
  printf("hdr->e_entry: %x\n", hdr->e_entry);
}
  ...
  for (int i=0 ; i < elf_header->e_phnum; ++i)
  {
    if (CHECK_PT_TYPE_LOAD(elf_pheader))
    {
      ...
      for (int i=0 ; i < elf_pheader->p_filesz ; ++i)
      {
        if (i % 16 == 0)
          printf("\n");
        //printf("%02d ", elf_pheader->p_offset+i);
        printf("%02x ", buf[elf_pheader->p_offset+i]);
      }
      printf("\n");
    }
    else
    {
      printf("%d ## is not loaded\n", i);
    }
    ++elf_pheader;
  }
